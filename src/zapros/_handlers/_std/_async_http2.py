...
from typing import TYPE_CHECKING, AsyncIterator, Callable, Union, cast
...
from ._conn import AsyncHttpConnection
...
async def _aiter_single(data: bytes) -> AsyncIterator[bytes]:
    yield data
...
class AsyncHttp2Connection(AsyncHttpConnection):
    """
    Locking discipline
    ------------------
    ...
    def __init__(
        ...
        self._init_lock = AnyLock()
        self._read_lock = AnyLock()
        self.state_lock = AnyLock()
        ...
        self.stream_limiter = AsyncStreamLimiter(limit=100)
    ...
    async def send_request(
        ...
        *,
        ...
        await self._ensure_initialized(write_timeout=write_timeout, deadline=deadline)
        ...
        try:
            if has_body:
                assert not is_sync_iterator(request.body) and request.body is not None
            ...
            await self.reset_stream(stream_id, write_timeout=write_timeout, deadline=deadline)
            raise

        return Response(
            status=status,
            headers=resp_headers,
            content=AsyncHttp2ResponseStream(self, stream_id, read_timeout=read_timeout),
            context={
                "network": {
    ...
    async def aclose(self) -> None:
        ...
        await self.stream_limiter.fail_all()
        try:
    ...
    async def _allocate_stream_and_send_headers(
        self,
        request: Request,
        *,
        has_body: bool,
        ...
        await self.stream_limiter.acquire()
        try:
            async with self.state_lock:
                try:
    ...
    async def _send_request_body(
        ...
        *,
        ...
        chunks: AsyncIterator[bytes] = _aiter_single(body) if isinstance(body, bytes) else body
        ...
        async for chunk in chunks:
            ...
            while offset < len(chunk):
                new_offset, data = await self._try_send_data_chunk(stream_id, chunk, offset)
        ...
        *,
        ...
        async with self._read_lock:
            ...
            if new_max_concurrent_streams is not None:
                await self.stream_limiter.update_limit(new_max_concurrent_streams)
    ...
    async def _receive_response_headers(
        ...
        *,
        ...
        while True:
            event = await self.receive_stream_event(
                stream_id,
                read_timeout=read_timeout,
                write_timeout=write_timeout,
                deadline=deadline,
...
class AsyncHttp2ResponseStream(AsyncClosableStream):
    def __init__(
        self,
    ...
    def __aiter__(self) -> AsyncHttp2ResponseStream:
        return self
    ...
    async def __anext__(self) -> bytes:
        ...
        while True:
            event = await self._conn.receive_stream_event(
                self._stream_id,
