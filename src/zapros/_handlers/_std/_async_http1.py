...
from zapros._handlers._std._conn import AsyncHttpConnection, BrokenConnectionError
...
class AsyncHttp1ResponseStream(AsyncClosableStream):
    """
    Byte stream over an h11 response body, with pooled connection lifecycle.

    Wraps an :class:`AsyncHttp1Connection` whose h11 state machine is positioned just after
    ...
    * the connection itself reports it can be reused.
    ...
    def __aiter__(self) -> AsyncHttp1ResponseStream:
        return self
...
class AsyncHttp1Connection(AsyncHttpConnection):
    ...
    async def send_request(
        ...
        *,
        ...
        if request.body is not None and not isinstance(request.body, (bytes, AsyncIterator)):
    ...
    async def _receive_response_headers(
        ...
        *,
        ...
        while True:
            ...
            if event is h11.NEED_DATA:
                data = await self.stream.read(DEFAULT_READ_SIZE, timeout=read_timeout)
                if not data:
